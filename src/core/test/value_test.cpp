...
TEST_F(value, fn) {
    ...
    EXPECT_NE(type, type_get_fn(&m_arena, i64_t, params_ar, 1, (FuncPtr)_plus, nullptr, false));
...
TEST_F(value, fn_ptr) {
    ...
    EXPECT_NE(type, type_get_fn_ptr(&m_arena, u64_t, params_ar));
...
TEST_F(value, struct) {
    ...
    EXPECT_NE(type, type_get_struct(&m_arena, fields_ar, 1));
