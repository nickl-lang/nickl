...
#define NK_HASH_TREE_IMPL(TTree, TItem, TKey, GetKeyFunc, KeyHashFunc, KeyEqualFunc)                      \
    typedef struct {                                                                                      \
        ...
        bool existing;                                                                                    \
                                                                                                          ...
                                                                                                          \
    static _##TTree##_SearchResult _##TTree##_findNode(_##TTree##_Node **node, TKey key, NkHash64 hash) { \
        while (*node) {                                                                                   \
...
#define NK_HASH_TREE_IMPL_K(TTree, TKey, KeyHashFunc, KeyEqualFunc) \
    static TKey _##TTree##_Item_GetKey(TKey const *key) {           \
        return *key;                                                \
    }                                                               \
...
#define NK_HASH_TREE_IMPL_KV(TTree, TKey, TVal, KeyHashFunc, KeyEqualFunc) \
    static TKey _##TTree##_Item_GetKey(TTree##_Item const *item) {         \
        return item->key;                                                  \
    }                                                                      \
