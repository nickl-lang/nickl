...
TEST_F(ir, nested_functions) {
    ...
    nkir_startFunct(
        p, getEight, cs2s("getEight"), nkt_get_fn(m_arena, i32_t, void_t, NkCallConv_Nk, false));
    nkir_startBlock(p, nkir_makeBlock(p), cs2s("start"));

    auto getFour = nkir_makeFunct(p);
    nkir_startFunct(
        p, getFour, cs2s("getFour"), nkt_get_fn(m_arena, i32_t, void_t, NkCallConv_Nk, false));
    nkir_startBlock(p, nkir_makeBlock(p), cs2s("start"));
