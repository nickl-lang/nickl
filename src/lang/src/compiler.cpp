...
#define X(TYPE, VALUE_TYPE) nkltype_t CAT(TYPE, _t);
NUMERIC_ITERATE(X)
#undef X
...
struct NklCompiler_T {
    ...
    std::unordered_map<nkid, usize> id2blocknum{};
...
nks irBlockName(NklCompiler c, char const *name) {
    ...
    char buf[1024];
    usize size = std::snprintf(buf, AR_SIZE(buf), "%s%zu", name, num);
...
void defineArg(NklCompiler c, nkid name, usize index, nkltype_t type) {
...
ValueInfo getIndex(NklCompiler c, ValueInfo const &lhs, ValueInfo const &index) {
    ...
    case NklType_Slice: {
        ...
        auto const mul = nkir_make_mul({}, asRef(c, index), asRef(c, makeValue<u64>(c, u64_t, nklt_sizeof(elem_t))));
        auto const offset_ref = asRef(c, makeInstr(mul, u64_t));
        auto const add = nkir_make_add({}, data_ref, offset_ref);
        return deref(c, asRef(c, makeInstr(add, elem_ptr_t)));
    }
...
ValueInfo import(NklCompiler c, fs::path filepath) {
    ...
    CHECK(defineComptimeConst(c, s2nkid({stem.c_str(), stem.size()}), makeValueComptimeConst(fn_val)));
    return fn_val_info;
}
...
#define X(TYPE, VALUE_TYPE)                                       \
    case CAT(n_, TYPE): {                                         \
        return makeValue<nkltype_t>(c, typeref_t, CAT(TYPE, _t)); \
    }
        NUMERIC_ITERATE(X)
#undef X
        ...
        case NkType_Ptr: {
            ...
            case NkType_Numeric: {
                if (src_type->as.num.value_type != Uint64) {
                    return error(c, "cannot cast any numeric type other than u64 to a pointer"), ValueInfo{};
                }
                return cast(dst_type, src);
            }
...
#define X(TYPE, VALUE_TYPE) \
    case VALUE_TYPE:        \
        return makeNumeric<TYPE>(c, CAT(TYPE, _t), text.data, "%" CAT(SCN, TYPE));
            NUMERIC_ITERATE(X)
#undef X
        default:
            assert(!"unreachable");
...
#define X(TYPE, VALUE_TYPE) \
    case VALUE_TYPE:        \
        return makeNumeric<TYPE>(c, CAT(TYPE, _t), text.data + 2, "%" CAT(SCNX, TYPE));
...
#undef X
    ...
    case n_call: {
        ...
        for (usize i = 0; i < arg_nodes.size; i++) {
            auto param_t =
                i < fn_t->as.fn.args_t->as.tuple.elems.size ? fn_t->as.fn.args_t->as.tuple.elems.data[i].type : nullptr;
    ...
    case n_comptime_const_def: {
        ...
        if (opt_link_tag && (def_id == n_fn_type || def_id == n_fn_type_var)) {
            ...
            CHECK(
                defineExtSym(c, s2nkid(sym_name), nkir_makeExtSym(c->ir, so, sym_name_with_prefix, tovmt(fn_t)), fn_t));
...
#define X(TYPE, VALUE_TYPE) CAT(TYPE, _t) = nkl_get_numeric(VALUE_TYPE);
    NUMERIC_ITERATE(X)
#undef X
