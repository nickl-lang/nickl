...
private:
    void reportError() {
        auto const err_msg = m_lexer.err.moveStr(m_arena);
        m_err_sb << "Error message: " << err_msg << "\n Tokens: { ";
        for (const auto &token : m_lexer.tokens) {
            m_err_sb << "'";
            string_escape(m_err_sb, token.text);
            m_err_sb << "', ";
        }
        m_err_sb << "}";
        auto str = m_err_sb.moveStr(m_arena);
        LOG_ERR("%.*s", str.size, str.data);
    }

    StackAllocator m_arena{};
    ARRAY_SLICE(char, m_err, 1000);
    StaticStringBuilder m_err_sb{m_err};
    nkl::Lexer m_lexer{m_err_sb};
};
